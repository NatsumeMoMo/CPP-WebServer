...
{
public:
    Server(const char *IP, uint16_t Port, int sep, int Threadnum = 10);
    ~Server();
    void serverLoop();
    void stop();
    // 改以下Connection为智能指针
    ...
    /* 回调过程为 Connection::handleMessage->Connection::closeCallback()->Server::closeConnection() */
...
private:
    ...
    std::vector<EventLoop *> subloops_;      // 子循环
