...
class Connection;
/*
EventLoop 主要是把Server中的Epoll对象和serverLoop中对各个Channel事件的执行循环封装为
一个类
*/
class EventLoop
{
...
public:
    EventLoop(bool ismainloop, int timetvl=30, int timeout=60);
    ...
    // 异步唤醒时间循环部分
    /*
        该部分在本项目中未被使用. 这部分是为了上层业务部分使用的工作线程池将任务投递到IO线程池中执行所准备的
        但由于本项目中HttpServer中并没有使用工作线程池，所以该部分代码未被使用。
    */
